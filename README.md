# LearningPytorch

### 根据李沐老师的课程进行学习

<img src="README.assets/image-20220323184636612.png" alt="image-20220323184636612" style="zoom:50%;" />

## 纸质文献与连接

[《动手学深度学习 PyTorch版》视频地址](https://space.bilibili.com/1567748478/channel/seriesdetail?sid=358497)

[《动手学深度学习》课本](https://zh-v2.d2l.ai/)

## 章节

### [Basic](./Basic)

* [性能测试](./Basic/performanceTest.py)

### [计算梯度](./计算梯度)

### [线性回归](./线性回归)

* [均方误差](./线性回归/均方误差.ipynb)
* [正态分布](./线性回归/正态分布.ipynb)
* [矢量化加速](./线性回归/矢量化加速.py)
* [线性回归](./线性回归/线性回归.py)
* [线性回归底层](./线性回归/线性回归底层.py)
* [线性回归框架](./线性回归/线性回归框架.py)

### [SoftMax](./softmax)

* [softmax底层](./softmax/softmax底层.py)
* [softmax框架](./softmax/softmax框架.py)

### [多层感知机](./感知机)

### [过拟合与欠拟合](./过拟合与欠拟合/overfittingAndUnderfitting.ipynb)

### [权重衰减](./WeightDecay/权重衰退.ipynb)

### [丢弃法](./dropout)

### [Kaggle:房价预测](./kaggleHouse)

### [PyTorch框架](./torch)

### [CNN](./CNN)

* [CNN底层](./CNN/CNN底层实现.ipynb)
* [LeNet](./CNN/LeNet.ipynb)
* [AlexNet](./CNN/AlexNet.py)
* [VGG](./CNN/VGG.ipynb)
* [NiN](./CNN/NiN.ipynb)
* [GoogLeNet](./CNN/GoogLeNet.ipynb)
* [BatchNorm](./CNN/BatchNorm.ipynb)
* [ResNet](./CNN/ResNet.ipynb)
* [DenseNet](./CNN/DenseNet.ipynb)
### [性能优化(GPU并行)](./computationalPerformance)

* [异步计算](./computationalPerformance/asynchronousComputation.ipynb)
* [混合编程](./computationalPerformance/hybridProgramming.ipynb)
* [GPU并行](./computationalPerformance/mutilGPU.ipynb)
* [自动并行](./computationalPerformance/automaticParallelism.ipynb)

### [GAN](./GAN)

* [BASIC_GAN](./GAN/BASIC.ipynb)

### [RNN](./RNN)
* [SequenctModel](./RNN/SequenctModel.ipynb)
* 
### [AttentionMechanism](./AttentionMechanism)
* [可视化Visualization](./AttentionMechanism/visualization.ipynb)
* [评价函数ScoreFunction](./AttentionMechanism/scoreFunction.ipynb)
